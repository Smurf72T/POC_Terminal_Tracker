...
public:
    ...
    // строки документа или сетки окна комплектов (серийники/imei1/imei2 — по
    // одному комплекту на индекс). Серийник заполняет ПЕРВЫЙ свободный комплект
    // (в окне комплектов строки заранее созданы под количество), при отсутствии
    // свободных — добавляет новый; голый IMEI дописывается к последнему
    // комплекту (сначала в IMEI 1, затем в IMEI 2), IMEI с подписью «2» —
    // сразу в слот IMEI 2. Возвращает true, если данные изменились.
