...
MainWindow::MainWindow(QWidget* parent) :
    QMainWindow(parent), ui(new Ui::MainWindow), topClientsModel(new QSqlQueryModel(this)),
    recentDocsModel(new QSqlQueryModel(this)), refreshTimer(new QTimer(this))
{
    ui->setupUi(this);
    ...
    connect(&DatabaseManager::instance(), &DatabaseManager::dataChanged, this, &MainWindow::onDatabaseDataChanged);
    ...
    connect(ui->tableViewTopClients, &QTableView::doubleClicked, this, &MainWindow::onTopClientDoubleClicked);
    ...
    connect(ui->tableViewRecentDocs, &QTableView::doubleClicked, this, &MainWindow::onRecentDocDoubleClicked);
    ...
    connect(m_opsScheduler, &OpsScheduler::backupFinished, this,
    ...
    connect(m_updater, &UpdateManager::noUpdateAvailable, this, [this]() {
        statusBar()->showMessage(QString("Обновлений нет (версия %1)").arg(m_updater->currentVersion()), 8000);
    });
    ...
    connect(m_updater, &UpdateManager::downloadProgress, this, [this](qint64 received, qint64 total) {
        if (total > 0)
            ...
            statusBar()->showMessage(QString("Скачивание обновления: %1 КБ").arg(received / 1024));
    });
    ...
    connect(m_updater, &UpdateManager::downloadFailed, this,
            [this](const QString& error) { QMessageBox::warning(this, "Скачивание обновления", error); });
    m_updater->start();
}
...
void MainWindow::setupUI()
{
    ...
    connect(ui->actionArchiveStatusChange, &QAction::triggered, this,
            &MainWindow::onActionArchiveStatusChange_triggered);
    ...
    connect(ui->actionExpiryNotifications, &QAction::triggered, this,
            &MainWindow::onActionExpiryNotifications_triggered);
...
void MainWindow::setupCharts()
{
    ...
    chartsGroup->setStyleSheet("QGroupBox { font-size: 14px; font-weight: bold; color: #CCCCCC; "
...
void MainWindow::loadCounters()
{
    ...
    if (query.exec("SELECT COUNT(*) FROM tblterminals") && query.next()) {
        updateCounterWidget(ui->labelValueTotal, ui->labelNameTotal, query.value(0).toString(), "Всего терминалов",
                            "#3498db");
    }

    if (query.exec("SELECT COUNT(*) FROM tblterminals WHERE status = 0") && query.next()) {
        updateCounterWidget(ui->labelValueFree, ui->labelNameFree, query.value(0).toString(), "Свободно терминалов",
                            "#2ecc71");
    }

    if (query.exec("SELECT COUNT(*) FROM tblterminals WHERE status = 1") && query.next()) {
        updateCounterWidget(ui->labelValueRented, ui->labelNameRented, query.value(0).toString(), "В аренде",
                            "#e74c3c");
    }

    if (query.exec("SELECT COUNT(*) FROM tblsimcards") && query.next()) {
        updateCounterWidget(ui->labelValueTotalSIM, ui->labelNameTotalSIM, query.value(0).toString(), "Всего SIM-карт",
                            "#9b59b6");
    }
    ...
    if (query.exec("SELECT COUNT(*) FROM tblsimcards s "
                   ...
                   ")") &&
        query.next()) {
        updateCounterWidget(ui->labelValueFreeSIM, ui->labelNameFreeSIM, query.value(0).toString(), "Свободно SIM",
                            "#1abc9c");
    }

    if (query.exec("SELECT COUNT(*) FROM tblclients") && query.next()) {
        updateCounterWidget(ui->labelValueClients, ui->labelNameClients, query.value(0).toString(), "Клиентов",
                            "#f39c12");
    }
}

void MainWindow::updateCounterWidget(QLabel* valueLabel, QLabel* nameLabel, const QString& value, const QString& label,
                                     const QString& color)
{
    if (valueLabel) {
...
void MainWindow::openFreeDevicesReport()
{
    ...
    connect(btnExportTerm, &QPushButton::clicked, [termView, dialog]() {
        ...
        if (!model)
            return;
        QString filePath = QFileDialog::getSaveFileName(dialog, "Экспорт свободных терминалов", "free_terminals.xlsx",
    ...
    connect(btnExportSim, &QPushButton::clicked, [simView, dialog]() {
        ...
        if (!model)
            return;
        QString filePath = QFileDialog::getSaveFileName(dialog, "Экспорт свободных SIM", "free_simcards.xlsx",
                                                        "Excel (*.xlsx);;Все файлы (*)");
        if (!filePath.isEmpty()) {
...
void MainWindow::openClientRentalReport(int clientId, const QString& clientName)
{
    ...
    QString queryStr = "SELECT m.modelname AS \"Модель\", "
    ...
    connect(btnExport, &QPushButton::clicked, [model, clientName, dialog]() {
        QString filePath = QFileDialog::getSaveFileName(
            dialog, "Экспорт отчёта", QString("terminals_%1.xlsx").arg(clientName.simplified().replace(' ', '_')),
            "Excel (*.xlsx);;Все файлы (*)");
        if (!filePath.isEmpty()) {
...
void MainWindow::ensureBackupWorker()
{
    ...
    connect(this, &MainWindow::backupRequested, m_backupWorker, &BackupWorker::createBackup, Qt::QueuedConnection);
    connect(this, &MainWindow::restoreRequested, m_backupWorker, &BackupWorker::restore, Qt::QueuedConnection);
    connect(m_backupWorker, &BackupWorker::backupFinished, this, &MainWindow::onManualBackupFinished);
    connect(m_backupWorker, &BackupWorker::restoreFinished, this, &MainWindow::onManualRestoreFinished);
}
...
void MainWindow::performBackup()
{
    ...
    QString filePath = QFileDialog::getSaveFileName(
        this, "Сохранить резервную копию БД",
    ...
    QString passphrase = QInputDialog::getText(
        this, "Пароль для шифрования бэкапа",
...
void MainWindow::performRestore()
{
    ...
    QString filePath = QFileDialog::getOpenFileName(this, "Выберите файл резервной копии", QString(),
                                                    "SQL файлы (*.sql);;Все файлы (*)");
    ...
    QMessageBox::StandardButton reply =
        QMessageBox::warning(this, "Подтверждение",
    ...
    bool passOk;
    QString passphrase = QInputDialog::getText(
        this, "Пароль бэкапа", "Введите passphrase бэкапа (пусто — для незашифрованного файла):", QLineEdit::Password,
        QString(), &passOk);
    if (!passOk)
        return;
...
void MainWindow::onActionIntegrityCheck_triggered()
{
    ...
    *conn = connect(m_opsScheduler, &OpsScheduler::integrityFinished, this, [this, conn](bool, const QString& summary) {
...
void MainWindow::onActionOpsLog_triggered()
{
    ...
    if (!QFile::exists(path)) {
        QMessageBox::information(this, "Журнал операций", "Журнал операций пока пуст.\nПуть: " + path);
        return;
    }
...
void MainWindow::showGlobalSearch()
{
    ...
    connect(input, &QLineEdit::textChanged, &dialog, [list, input]() {
        ...
        query.prepare(
            "SELECT clientid, clientname FROM tblclients WHERE clientname ILIKE :q OR inn ILIKE :q2 LIMIT 10");
        query.bindValue(":q", like);
        query.bindValue(":q2", like);
        if (query.exec()) {
            while (query.next())
