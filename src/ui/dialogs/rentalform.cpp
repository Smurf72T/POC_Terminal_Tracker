...
void RentalForm::loadFreeSIMsToDelegate()
{
    ...
    // Добавляем SIM, уже установленные в свободные терминалы (документ
    // «Установка SIM»), чтобы их можно было видеть/выбирать в редакторе.
...
void RentalForm::autoFillSimForTerminal(int row, int terminalId)
{
    if (terminalId <= 0) {
        // Терминал не выбран — очищаем SIM
