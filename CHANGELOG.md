...
- **Строка документа = модель + кол-во + списки номеров**: табличная часть
  ...
  «IMEI 1», «IMEI 2») вместо одной строки на терминал; модель и «Сформировать
  строки» убраны из шапки формы (`receiptform`)
- **Окно ввода списка** (`src/ui/dialogs/seriallistdialog.*`): двойной клик по
  ячейке «Серийные номера»/«IMEI» открывает окно с текстовым полем «один номер
  на строку», живым счётчиком «N из M», проверкой дублей и длины IMEI (15 цифр,
