...
### После рефакторинга (оценка)

| Файл | Строк (оценка) | Сокращение |
|------|----------------|------------|
| receiptform.cpp | ~300 | -492 (-62%) |
| rentalform.cpp | ~280 | -426 (-60%) |
| statuschangeform.cpp | ~250 | -383 (-61%) |
| returnform.cpp | ~230 | -374 (-62%) |
| paymentform.cpp | ~200 | -216 (-52%) |
| **base/documentdialog.cpp** | ~120 | НОВОЕ |
| **base/printservice.cpp** | ~80 | НОВОЕ |
| **base/transactionguard.cpp** | ~50 | НОВОЕ |
| **services/documentnumbergenerator.cpp** | ~20 | НОВОЕ |
| **services/postactionlogger.cpp** | ~20 | НОВОЕ |
| **services/simcardservice.cpp** | ~40 | НОВОЕ |
| **services/statuschangeservice.cpp** | ~30 | НОВОЕ |
| **services/serialunitsservice.cpp** | ~40 | НОВОЕ |
| **Итого** | **~1 440** | **-54%** |
...
1. [ ] Все 5 файлов ≤ 400 строк
2. [ ] `cmake --build .` проходит без ошибок
3. [ ] `ctest --output-on-failure` проходит без ошибок
4. [ ] Проектный линтер проходит без ошибок (если настроен)
5. [ ] Ручное тестирование: создание/редактирование/проведение/печать каждого типа документа
6. [ ] Все новые файлы имеют unit-тесты (для сервисов)
7. [ ] Git-история: каждый этап — отдельный коммит с осмысленным сообщением
