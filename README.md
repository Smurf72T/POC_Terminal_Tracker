...
- **Документы:**
  ...
  - Установка SIM в терминал (комплектация на складе)
...
3. Учётная запись `admin` создаётся автоматически со **случайным одноразовым паролем**,
   который выводится один раз в консоль при первом запуске (смените его при первом входе).
   Подробнее: `docs/OPS.md` §1.2.
...
Руководство по сборке, тестам и правилам PR: [CONTRIBUTING.md](CONTRIBUTING.md).
